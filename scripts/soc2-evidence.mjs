...
### WHAT RE-ENTERING A VARIABLE CAN UNDO

**A fix applied in a dashboard is a fix that the next edit to the same dashboard
can quietly reverse.** Recorded here because both halves happened within hours
of each other on 2026-09-12, and neither would have been visible from the
repository.

**One entry covering two environments is the same sharing in a new row.** After
the first pass, \`CUSTOMER_SESSION_SECRET\`, \`PARTNER_SESSION_SECRET\` and
\`MFA_ENCRYPTION_KEY\` each read "Production and Preview" as a SINGLE entry. That
is not a split. Two environments selected on one value share that value exactly
as All Environments did, and the row looks different while nothing changed. They
are being split into two entries with different values.

**And a variable can leave the list entirely.** \`OPS_SESSION_SECRET\` was the
only one of the four ever split correctly, and after the same pass it was no
longer in the list at all. If it was deleted, staff sign in stops: the symptom
is an outage rather than a weakness, because \`signingKey()\` returns null with
no fallback, so no session can be forged because none can be signed.
Announcements stop too, because an unsubscribe link cannot be signed and
\`ops-announce\` refuses to send rather than send an unsigned one.

**The lesson for this register:** an environment variable list is a surface
where a correct state and an incorrect one look nearly identical, where the fix
and the regression are the same gesture, and where nothing in this repository
can see either. That is why the declaration records what was read and when, why
the board fails while a secret is declared shared, and why it will keep failing
until somebody writes down that it was fixed.

**The remediation text in the product was part of the problem.** When a preview
is caught pointed at production, \`db-guard.ts\` tells the reader what to scope,
and until 2026-09-12 that sentence named ONE identity secret of four. Somebody
following it exactly would have split a quarter of what needed splitting, which
is close to what had happened.
