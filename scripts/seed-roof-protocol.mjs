...
const EMIT_DISCIPLINE = process.argv.includes("--emit-discipline-sql");
...
const EMITTED_DISCIPLINE = "supabase/migrations/0056_the_engineer_states_the_discipline.sql";

const { RC001, RC001_VERSIONS, RC001_SIGNATURE_EVIDENCE } = await import("../src/content/protocols/rc-001.ts");

/*
 * THE WRITTEN DIRECTION BEHIND THE DISCIPLINE, looked up rather than retyped.
 *
 * 0056 quotes the engineer in its own header, and a quotation typed into a
 * generator is a second copy of a licensee's words that nothing compares. This
 * reads the one `compliance-audit` already digests against the file on disk.
 */
const { engineerDirections } = await import("../src/config/engineer-directions.ts");
const RC001_DISCIPLINE_DIRECTION =
  engineerDirections.find((d) => d.key === "roof-certification-discipline") ?? null;
...
/*
 * ===========================================================================
 * --emit-discipline-sql WRITES 0056: THE DISCIPLINE, ONTO THE EXISTING ROW.
 * Operator ruling, 2026-09-21.
 * ===========================================================================
 *
 * SEPARATE FROM 0055 BECAUSE THEY ARE SEPARATE ACTS BY SEPARATE PEOPLE. 0055
 * records provenance: the operator putting a signed document into the platform.
 * This records a JUDGEMENT by the engineer of record, given in writing on
 * 2026-09-21 and held on disk. Folding it into 0055 would have meant either
 * holding the document back for five days, or writing his answer before he gave
 * it.
 *
 * THE VALUE IS READ FROM THE REGISTRY, NEVER TYPED HERE, the same rule 0055's
 * digest follows. `RC001.requiresDiscipline` is the one home, it is sourced
 * from his email, and `protocol-registry-audit` reads the literal back OUT of
 * the migration and asserts it equals the registry.
 *
 * AND IT REFUSES TO EMIT A NULL. A migration setting requires_discipline to
 * null is a migration that does nothing while looking like it did something,
 * and it would be the exact shape of a green over an empty set.
 *
 * IT UPDATES ONLY WHILE awaiting_engineer. Proven in migration-audit's
 * replayed database that the column is NOT frozen at publication, so this
 * could land either side of his approval. It is still written narrowly: the
 * engineer stated the discipline of a protocol he has not yet approved, and
 * the migration should describe that state rather than a wider one it was
 * never asked for.
 */
if (EMIT_DISCIPLINE) {
  if (APPLY || EMIT_SQL) {
    console.error("STOP: --emit-discipline-sql writes one file. Run it on its own.");
    process.exit(1);
  }

  if (!RC001_DISCIPLINE_DIRECTION) {
    console.error("");
    console.error("STOP: no written direction keyed roof-certification-discipline is on record.");
    console.error("");
    console.error("The discipline is the engineer's judgement and this migration quotes him for it.");
    console.error("Record the direction, with its evidence on disk, before emitting anything.");
    process.exit(1);
  }

  if (row.requires_discipline === null || String(row.requires_discipline).trim() === "") {
    console.error("");
    console.error("STOP: RC001.requiresDiscipline is null, so there is no discipline to write.");
    console.error("");
    console.error("This is the engineer's answer and it is never inferred from the service line's");
    console.error("name. Until he states it, roof-inspections stays a waitlist and that is correct.");
    process.exit(1);
  }

  const sql = `/*
 * ===========================================================================
 * 0056  THE ENGINEER OF RECORD STATES THE DISCIPLINE ${RC001.documentNumber} REQUIRES.
 * ===========================================================================
 *
 ...
 * from the engineer's written direction of ${RC001_DISCIPLINE_DIRECTION.on},
 ...
 *
 * HIS WORDS: "${RC001_DISCIPLINE_DIRECTION.quote}"
 *
...
begin
  ...
  where document_number = ${sqlLiteral(row.document_number)}
    ...
    and document_sha256 = ${sqlLiteral(row.document_sha256)}
    and status          = 'awaiting_engineer';
  if n <> 1 then
    raise exception
      'eng: expected exactly one % v% awaiting its engineer under the declared digest, found %.',
      ${sqlLiteral(row.document_number)}, ${sqlLiteral(row.version_label)}, n;
  end if;
end;
$$;

update eng_protocol_templates
   set requires_discipline = ${sqlLiteral(row.requires_discipline)}
 where document_number = ${sqlLiteral(row.document_number)}
   and version_label   = ${sqlLiteral(row.version_label)}
   and status          = 'awaiting_engineer';
...
begin
  ...
  where document_number    = ${sqlLiteral(row.document_number)}
    and version_label      = ${sqlLiteral(row.version_label)}
    and requires_discipline = ${sqlLiteral(row.requires_discipline)};
  if n <> 1 then
...
`;
  ...
  const existed = existsSync(EMITTED_DISCIPLINE);
  writeFileSync(EMITTED_DISCIPLINE, sql, "utf8");
  console.log(`${existed ? "REWROTE" : "WROTE"} ${EMITTED_DISCIPLINE}`);
  console.log(`discipline literal ${row.requires_discipline}, from ${RC001_DISCIPLINE_DIRECTION.on}`);
  console.log("");
