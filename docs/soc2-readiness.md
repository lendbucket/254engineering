...
| A second factor, TOTP, with recovery codes, and a per role requirement. | src/lib/ops-mfa.ts and src/lib/totp.ts, where 6 digits and a 30 second period are constants, checked against RFC 6238's published vectors by scripts/proofs/totp-matches-the-rfc.mjs, which scripts/proofs-audit.mjs runs on every board. | `eng_mfa_enrolments, eng_mfa_recovery_codes, eng_roles.mfa_requirement` | yes | yes |
