...
**Generated by `scripts/soc2-evidence.mjs` on 2026-09-12 21:08:22 against development (ythzaiqeoijlrdibnieo) (`ythzaiqeoijlrdibnieo`). Do not edit this file. Every figure below was read at generation time.**
...
Read from `ythzaiqeoijlrdibnieo` at 2026-09-12 21:08:22.
...
| Audit trail events | 12848 |
| Earliest audit event | 2026-09-02 18:39:59 |
| Latest audit event | 2026-09-12 21:20:07 |
...
| --- | --- | --- | --- |
| `SUPABASE_SERVICE_ROLE_KEY` | Everything. It bypasses row level security on every table, and since there are zero policies it is the ONLY way in. Holding it is equivalent to holding the database. | Vercel for production; .env.local for development, where it is the DEVELOPMENT project's key and every audit reads through it. Never in the working tree for production. WHETHER THE PRODUCTION KEY IS SCOPED TO THE PRODUCTION ENVIRONMENT ALONE IS UNKNOWN, pending the operator reading Vercel: a Preview inheriting it is the exact hazard previewPointingAtProduction() exists for, and it has happened once already, on 2026-09-03. | Never. |
...
| `RESEND_API_KEY` | Sending mail as this firm's domain, and reading the delivery log. | Vercel for production. REMOVED from .env.local 2026-09-12: nothing local needs it and its presence made two bugs send 55 real emails. Whether it is set on Preview is UNKNOWN, pending the operator reading Vercel. | Never. |
