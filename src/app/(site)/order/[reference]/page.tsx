    ...
    return (
      <Container>
        <div className="mx-auto max-w-[62ch] py-20">
          <h1 className="font-display text-[1.8rem] leading-[1.2] font-semibold text-slate">
            This link does not open an order
          </h1>
          <p className="mt-4 text-[1.02rem] leading-[1.7] text-slate-muted">
  ...
  return (
    <Container>
      <div className="mx-auto max-w-[70ch] py-14 sm:py-20">
        ...
        <p className="mt-2 text-[1.02rem] leading-[1.7] text-slate-muted">
          {view.propertyAddress}
          {view.city ? `, ${view.city}` : ""}, {view.county} County
        </p>

        {paid ? (
          <div className="mt-6 rounded-[4px] border border-[#bcdcc7] border-l-[3px] border-l-[#2f7a4f] bg-[#e8f3ec] px-4 py-3.5">
            <p className="text-[15px] font-semibold text-[#14522f]">Payment received</p>
            <p className="mt-1 text-[14px] leading-[1.6] text-[#14522f]">
        ...
        {cancelled ? (
          <div className="mt-6 rounded-[4px] border border-[#f0d9a8] border-l-[3px] border-l-brass bg-[#fdf3e0] px-4 py-3.5">
            <p className="text-[15px] font-semibold text-[#7a4c05]">Nothing was charged</p>
            <p className="mt-1 text-[14px] leading-[1.6] text-[#7a4c05]">
        ...
        {view.refunded ? (
          <div className="mt-6 rounded-[4px] border border-limestone-line border-l-[3px] border-l-slate bg-white px-5 py-4">
            <p className="text-[15px] font-semibold text-slate">
              {view.refunded.amount} has been refunded
            </p>
            <p className="mt-1.5 text-[14px] leading-[1.65] text-slate-muted">
        ...
        <section className="mt-10">
          <h2 className="text-[13px] font-bold tracking-[0.1em] text-brass-ink uppercase">
            Where it is
          </h2>
          <p className="mt-2 text-[1.02rem] leading-[1.7] text-slate">{view.statusLine}</p>
        </section>
          ...
          <section className="mt-10">
            ...
            <ol className="mt-4 flex flex-col gap-4">
              {view.timeline.map((entry) => (
                <li key={entry.at} className="border-l-[2px] border-limestone-line pl-4">
                  ...
                  <p className="mt-1 text-[15px] leading-[1.6] text-slate">{entry.summary}</p>
                </li>
              ))}
            </ol>
          </section>
        ...
        <section className="mt-10">
          ...
          <dl className="mt-4 border-t border-limestone-line">
            {view.lines.map((line) => (
              <div
                key={line.label}
                className="flex justify-between border-b border-limestone-line py-2.5"
              >
                <dt className="text-[15px] text-slate-muted">{line.label}</dt>
                <dd className="text-[15px] text-slate">{line.amount}</dd>
              </div>
            ))}
            <div className="flex justify-between py-2.5">
              <dt className="text-[15px] font-semibold text-slate">Total</dt>
              <dd className="text-[15px] font-semibold text-slate">{view.total}</dd>
            </div>
          </dl>
        </section>
          ...
          <section className="mt-10">
            ...
            <ul className="mt-4 flex flex-col gap-2">
              {view.receives.map((line) => (
                <li key={line} className="text-[15px] leading-[1.65] text-slate-muted">
                  {line}
                </li>
              ))}
            </ul>
        ...
        {view.refundDisclosure ? (
          <section className="mt-10 border-t border-limestone-line pt-8">
            <h2 className="text-[13px] font-bold tracking-[0.1em] text-brass-ink uppercase">
            ...
            <div className="mt-4 flex flex-col gap-3">
              {view.refundDisclosure.split("\n\n").map((para) => (
                <p key={para} className="text-[14.5px] leading-[1.7] text-slate-muted">
                  {para}
                </p>
              ))}
            </div>
