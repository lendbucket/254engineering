...
export function StatusClient({ errors, windowMinutes }: { errors: Fault[]; windowMinutes: number }) {
  ...
  return (
    <div>
        ...
        {errors.map((f) => (
          <li key={f.fingerprint} className="py-3.5 first:pt-0 last:pb-0">
            <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1.5">
              <span className="text-[14px] font-semibold text-slate">{f.title}</span>
              {f.inWindow > 0 ? (
                <span className="text-[12.5px] font-semibold text-[#8c1d18]">
                  {f.inWindow} in the last {windowMinutes} min
                </span>
              ) : null}
              <span className="text-[12.5px] text-slate-muted">{f.occurrences} in total</span>
              {f.muted ? <Chip label="Muted" tone="neutral" /> : null}
            </div>

            <p className="mt-1 max-w-[80ch] font-mono text-[12px] leading-[1.5] break-all text-slate-muted">
              {f.fingerprint}
            </p>
      ...
      <p className="mt-3 max-w-[80ch] text-[12.5px] leading-[1.55] text-slate-muted">
